...
* **HU-229**: Como desarrollador backend, quiero configurar el backend del módulo Person con Java y Springboot, para poder implementar las funcionalidades específicas para el módulo.
* **HU-230**: Como desarrollador backend, quiero desarrollar las funciones de CRUD para el módulo Person, para poder estructurar la información requerida en la base de datos y poder realizar el manejo de datos por parte del módulo.
* **HU-231**: Como equipo de desarrollo, quiero desplegar la versión básica del módulo Person en un entorno local de pruebas con Postman, para verificar el funcionamiento de manejo de datos del módulo.
...
## ÉPICA 5: Análisis y Estadísticas

### Sprint 10

* **HU-401**: Como estudiante, quiero consultar mi promedio académico general, para conocer mi rendimiento.
* **HU-402**: Como estudiante, quiero consultar mis notas y compararlas con el promedio del grupo, para conocer mi posición académica.

### Sprint 11

* **HU-403**: Como estudiante, quiero ver un gráfico de mi progreso académico por semestre, para identificar mejoras o retrocesos.
* **HU-404**: Como docente, quiero ver estadísticas de notas por materia, para identificar el rendimiento global de los estudiantes.
* **HU-405**: Como docente, quiero ver estadísticas de notas por estudiante, para evaluar el progreso individual.
* **HU-406**: Como docente, quiero ver estadísticas de distribución de notas (promedio, mínimo, máximo, desviación estándar), para analizar tendencias.
* **HU-407**: Como docente, quiero filtrar estadísticas por fecha (cortes), para hacer un análisis temporal.
* **HU-408**: Como docente, quiero generar estadísticas de notas por corte, para analizar cómo evoluciona el rendimiento en el semestre.
* **HU-409**: Como docente, quiero identificar estudiantes en riesgo (bajo promedio), para tomar decisiones preventivas.
* **HU-410**: Como docente, quiero ver gráficos de barras por materia, para visualizar los promedios de manera clara.
* **HU-411**: Como docente, quiero ver gráficos circulares por grupo, para observar la distribución de calificaciones.
* **HU-412**: Como docente, quiero alternar entre diferentes tipos de gráficos (líneas, barras, circulares), para personalizar la visualización.
* **HU-413**: Como docente, quiero generar reportes comparativos entre cortes de un mismo estudiante, para ver cómo evoluciona su desempeño.
* **HU-414**: Como sistema, quiero calcular automáticamente promedios generales por semestre, para consolidar los datos en un solo valor.
* **HU-415**: Como coordinador, quiero ver estadísticas comparativas entre grupos, para identificar diferencias de desempeño académico.
* **HU-416**: Como sistema, quiero generar automáticamente un listado de estudiantes con notas menores a un umbral definido, para facilitar la detección de bajo rendimiento.

### Sprint 12

* **HU-417**: Como coordinador, quiero generar estadísticas globales de un programa académico, para tener una visión consolidada del rendimiento.
* **HU-418**: Como sistema, quiero permitir exportar estadísticas en formato CSV, para que puedan ser reutilizadas en otros sistemas.
* **HU-419**: Como coordinador, quiero filtrar estadísticas por materia específica, para analizar casos particulares.
* **HU-420**: Como coordinador, quiero ver reportes de rendimiento histórico (varios semestres), para analizar tendencias a largo plazo.
* **HU-421**: Como administrador, quiero generar reportes en PDF que incluyan tablas y gráficos de las estadísticas, para presentarlos en reuniones académicas.
* **HU-422**: Como usuario, quiero personalizar los reportes seleccionando qué materias, grupos o estudiantes incluir, para obtener información relevante.
* **HU-423**: Como usuario, quiero agregar un logo institucional en mis reportes, para darle un formato oficial.
* **HU-424**: Como coordinador, quiero descargar reportes consolidados por facultad o programa, para tener una visión estratégica de los resultados.
* **HU-425**: Como coordinador, quiero programar la generación automática de reportes semanales, para no tener que solicitarlos manualmente.
* **HU-426**: Como administrador, quiero ver métricas globales (número de estudiantes, docentes y universidades activas), para conocer el estado del sistema.
* **HU-427**: Como administrador, quiero ver gráficos de distribución (ejemplo: estudiantes por universidad o programa), para identificar patrones.

## ÉPICA 6: Carga de Archivos

### Sprint 13

* **HU-501**: Como docente, quiero descargar una plantilla de Excel para estudiantes, con estructura validada, para garantizar un formato estándar.
* **HU-502**: Como docente, quiero cargar un archivo Excel con estudiantes y que el sistema ejecute un POST hacia el backend, para registrar automáticamente los datos.
* **HU-503**: Como docente, quiero recibir un mensaje de error si el archivo de estudiantes no cumple con el formato esperado, para corregirlo.
* **HU-504**: Como docente, quiero cargar un archivo Excel con calificaciones, para que el sistema lo procese y almacene los datos en una base estructurada.
* **HU-505**: Como sistema, quiero validar que el archivo Excel tenga encabezados correctos (materia, estudiante, nota), para asegurar la integridad de los datos.
* **HU-506**: Como docente, quiero cargar un archivo PDF con calificaciones, para que el sistema pueda interpretar y extraer los datos relevantes.
* **HU-507**: Como sistema, quiero convertir automáticamente las tablas de calificaciones en PDF a un formato estructurado, para facilitar el análisis posterior.
* **HU-508**: Como sistema, quiero validar que los archivos subidos tengan el formato correcto (Excel o PDF), para evitar errores de procesamiento.
* **HU-509**: Como docente, quiero recibir un mensaje de error claro cuando suba un archivo no válido, para entender qué debo corregir.
* **HU-510**: Como sistema, quiero identificar automáticamente materias, estudiantes y cortes dentro de los archivos subidos, para organizar los datos sin intervención manual.
* **HU-511**: Como docente, quiero revisar una vista previa de los datos procesados, para confirmar que la información fue interpretada correctamente.
* **HU-512**: Como administrador, quiero que los datos extraídos se almacenen en una base de datos relacional, para poder consultarlos posteriormente.
* **HU-513**: Como sistema, quiero detectar errores en los documentos (datos faltantes o inconsistentes), para informar al usuario de problemas en la carga.
* **HU-514**: Como usuario, quiero que se guarde un historial de archivos subidos, para rastrear las cargas anteriores.